    ...
    return {
      navigate(params) {
        ...
        if (isRegexpStringMatch(externalUrlRegex, params.itemUrl)) {
          ...
          const url = new URL(params.itemUrl, window.location.origin);
...
function useSearchParameters({contextualSearch, ...props}) {
  ...
  let pluginSpecificFilter = [];
  if (location.pathname.includes('/cfdbfc79-7598-4828-6b2a-08dc27e3fe33/')) {
    pluginSpecificFilter = ['docusaurus_tag:docs-cfdbfc79-7598-4828-6b2a-08dc27e3fe33-current'];
  } else if (location.pathname.startsWith('/docs/') || location.pathname.startsWith('/en/docs/')) {
    pluginSpecificFilter = ['docusaurus_tag:docs-public-current'];
  }

  const facetFilters = contextualSearch
    ? // Merge contextual search filters with config filters
      mergeFacetFilters(pluginSpecificFilter, configFacetFilters)
    : configFacetFilters;
