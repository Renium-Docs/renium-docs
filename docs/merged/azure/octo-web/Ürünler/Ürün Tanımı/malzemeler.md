---
sidebar_position: 50
---
# Malzemeler

Ürün tipi  ```Malzemeler``` seçili ürünleri görütünlenir ve tanımlanır.



## İşlemler

### Yeni

### Düzenle

### Sil

### Insert Ürün Eşleştir



## Alanlar

### Ürün Tipi

```Malzemeler``` varsayılan gelir ve değiştirlemez.

### Ürün Alt Tipi

```Malzemeler``` ürün tipindeki alt tipler seçilebilir.


### Ürün Kodu  

Otomatik oluşturulur.

```<Konf. Ürün Tipi 2 No> . <Konf. Ürün Tipi 3 No> . <Konf. Ürün Tipi 4 No> . <Konf. Ürün Tipi 2 No> . <Renk> . <Konf. Ürün Tipi 5 No> . <Konf. Ürün Tipi 6 No> ```

### Beytuğ Ürün Kodu  

Otomatik oluşturulur.

```<Konf. Ürün Tipi 3 Kodu> | <Konf. Ürün Tipi 4> | <Konf. Ürün Tipi 5> | <Konf. Ürün Tipi 6> | <Ebat> | <Renk> ```

### Ürün Adı

```<Konf. Ürün Tipi 3 Adı> | <Konf. Ürün Tipi 4> | <Konf. Ürün Tipi 5> | <Konf. Ürün Tipi 6> | <Ebat*> | <Renk**> ```

*Ebat tanımı işaretli ise yazılır.

** Renk tanımı işaretli ise yazılır.

### Özellikler

#### Konf Ürün Tipi

#### Konf Ürün Tipi2

#### Konf Ürün Tipi3

#### Konf Ürün Tipi4

#### Konf Ürün Tipi5

#### Konf Ürün Tipi6

#### Konfeksiyon Ambalaj Tipi


#### Ebat



##### Ebat Tanım Niteliği

Evet ise Ebat tanımı ürün adına girer.

Hayır ise girmez.


#### Renk


:::tip

[Detaylı bilgi için ->](/docs/octo-web/Ürünler/Ana%20Tanımlar/renkler.md)

:::


##### Renk Tanım Niteliği

Evet ise Renk tanımı ürün adına girer.

Hayır ise girmez.

#### Ambalaj Ürünü
